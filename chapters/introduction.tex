...
\todo[inline,color=green]{I have structured the content of \cref{sec:contributions} by introducing this section.}
...
I present a \defn{\gls{SymbolPrecedenceRecommender}} that sorts the symbols by a linear combination\todo[color=red]{This is not true. Correct.}\todo{Stress the challenge of designing the proxy task. "We present a p. recommender which uses ... over syntactic features.} of their syntactic features using learned weights.
...
In both cases, I trained the recommender to generate \glspl{sp} that improve the likelihood of a\todo{an early?} successful proof search.

...
or automate feature extraction using a \gls{rnn} \cite{DBLP:conf/cade/ChvalovskyJ0U19,DBLP:conf/iclr/EvansSAKG18} or a \gls{gnn} \cite{DBLP:conf/cade/JakubuvCOP0U20}.\todo[author=MS,color=yellow]{konci toho odstavce nejak chybi nejaky zaver. Jakoze neco si muzem vybrat, ale co to ovlivni?}
...
% Deepire
DBLP:conf/cade/000121a,DBLP:conf/frocos/Suda21}.\todo{Reference TRAIL?}
...
such as mathematics and software verification, and have been encoded by various procedures\todo[author=MS,color=yellow]{poznamenal jsem si "not well connected" Asi by to chtelo o epsilon vic rozvest/zpresnit, aby bylo jasnejsi, proc obe casti vety argumentuji pro to same}.
...
or all literals that are maximal with respect to the \gls{to} are selected.\todo[author=MS,color=yellow]{Literal selection is another standalone restriction of the calculus, which just needs to be compatible with the ordering for completeness. Tedy rikat, ze ordering (jen ten) restringuje kalkulus dvema zpusoby je nepresne.}

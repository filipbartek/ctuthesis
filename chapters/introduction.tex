...
We then also call the conjecture a \defn{theorem}\todo[color=green]{\enquote{Theorem} is actually a \emph{provable} statement. A valid statement need not be provable in general.} of the \defn{theory}\todo{Define \enquote{theory} properly.} defined by the axioms,
or a \defn{logical consequence} of the axioms.
...
\todo[inline,color=green]{Consider discussing RL and looping here. With RL, we can use the prover as a black box, training on just the output status or runtime, with no introspection. This is done notably in SMAC.
...
\todo[inline,color=green]{What is the state of the art in strategy scheduling? Should we describe it for the sake of completeness? MS: Not necessarily. Consider adding a section.}

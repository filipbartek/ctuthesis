...
\todo[color=green]{Updated content starts here.}The search starts by initializing the processed set as empty and populating the unprocessed set with the input clauses.
In each iteration,
...
For example, the superposition calculus \cite{DBLP:journals/logcom/BachmairG94}\todo[color=green]{Here I omitted "ordered resolution" completely.}
...
i.e., it selects either a negative literal or all literals that are maximal with respect to a \gls{to}\todo[color=green]{Does it have to be a simplification ordering?},
...
in exchange for a narrower and faster-converging proof search.\todo[color=green]{Updated content ends here.}
...
Traditionally, finding a good configuration of one or more parameters\todo{Foreshadow strategy scheduling in this section.} of a prover is the work of an expert user that draws from their experience with \gls{atping}.
...
I used the \gls{fol} fragment of \gls{tptp}\todo{Consider discussing the relation between CASC and TPTP somewhere.} as the target\todo{Consider factoring out "target".} problem distribution\todo{Replace or clarify?} in most of my research.
...
Vampire uses a \gls{sot} \cite{DBLP:journals/logcom/BachmairG94}\todo[color=green]{Is the citation correct?} to restrict the proof search without compromising completeness:
\begin{enumerate}

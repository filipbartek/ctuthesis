...
One of these considerations is the choice of a suitable representation of the input data.\todo[color=green]{new sentence}
...
Each approach presents a specific tradeoff between representation faithfulness and computational efficiency.\todo[color=green]{new sentence}
...
\todo[color=green]{new subsection title}
...
and each encoding procedure may use a different symbol naming scheme.\todo[color=green]{new sentence}
...
which is parameterized by a \gls{sot}\todo{Consider citing Term rewriting and all that.} and a literal selection function\todo[color=green]{How is the selection function chosen in Vampire? I just want to know it, not necessarily write about it.}.
These parameters affect two restrictions of the inferences in the calculus: literal selection and equation orienting.\todo[color=green]{paragraph reworded and expanded}

\paragraph{Literal selection}
Each of the inference rules targets\todo[color=green]{Each inference operates on all literals by the means of substitution, but only some (\enquote{targeted}) literals by other means. I came up with \enquote{targeting} because I did not know any established term. Is this terminology ok?} some of the literals of the input clauses;
for example, the resolution rule targets (and resolves) a positive literal in a clause and a complementary negative literal in another\todo{possibly the same} clause.
...
the restriction does not compromise completeness of the calculus.\todo[color=green]{paragraph reworded and expanded}
...
Choosing a good \gls{to}\todo[color=green]{Here we silently assume that the ordering influences the selection function, and omit the discussion of how the selection function is chosen.} for a given input problem

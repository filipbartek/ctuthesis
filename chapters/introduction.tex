...
% Notes from CB:
% ZF (set theory) FOL was the formal foundation.
...
and \gls{atping}\todo{Automatic, or automated?}.\todo{This paragraph is too vague. What is reasoning?}
...
The standard way to define proof of unsatisfiability (or, more generally, proof of entailment) is based on an \defn{inference calculus}---a set of inference rules.
...
If each inference rule in the calculus is \defn{sound},
...
The clauses in the \defn{unprocessed set} have been inferred from the input clauses and have not yet been used as premises of inferences.
%deriving new facts,
...
For this reason, a successful inference calculus restricts inferences as much as possible while maintaining completeness.\todo{Too strong unsupported claim?}
...
that is, it selects either a negative literal or all literals that are maximal with respect to a \gls{to}\todo{Does it need to be the same ordering as the one used for equation orienting in the superposition rule?},
...
Each of the main decision points in a theorem prover is governed by a \defn{heuristic}---a procedure that aims to resolve the decision so as to optimize the prover's performance.\footnote{Standard performance measures include success rate and \gls{par} \cite{DBLP:journals/ai/BischlKKLMFHHLT16}. Each of them is parameterized by a runtime limit and a set of input problems.}
...
For example, \acrshort{ml}-based systems have been trained to perform clause selection directly \cite{
% ENIGMA
...
Although there is a growing body of research on \gls{ml} for \gls{atping} \cite{DBLP:conf/birthday/BlaauwbroekCGJK24},
...
Although the experiments presented in the following have been performed using \gls{vampire},
...
For these reasons, the problems do not share a common \gls{signature}---a symbol may represent different concepts across problems, and a concept may appear under different names.\todo{Include examples.}
...
The superposition inference rule generalizes the idea of rewriting terms by replacing equals with equals.\todo{This doesn't sound like my English.}
...
\Gls{kbo}, \gls{vampire}'s default \gls{to}\todo{Say something more about the ordering. Why is it a good choice?}, is parameterized by a \defn{\gls{sp}}---a permutation of the \gls{signature} of the input problem.
...
The configuration of all the options of a prover constitutes a \defn{strategy}.
...
that are relevant to \acrlong{atping} as well as to other forms of parameterized problem solving:
...
\item[Budgeted \gls{AlgorithmScheduling}] \cite{DBLP:journals/ec/KerschkeHNT19} is the task of finding a good algorithm schedule (a mapping from algorithms to runtime limits) given a distribution of problems, a portfolio of algorithms, and a runtime budget.
\end{description}

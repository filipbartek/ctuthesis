...
for resolving\todo{Consider using a different word: reduce, suppress, curb}
...
In the 20th century,
\gls{fol} emerged as the standard formalism for expressing these foundations \cite{DBLP:journals/bsl/Ferreiros01}.
...
DBLP:conf/atal/LibalN21}.\todo{The listed areas are actually related primarily to automated reasoning rather than \gls{fol}.}
...
A reasoning task is represented in \gls{fol} by a set of \defn{axioms} (for example, axioms of set theory, group theory, or arithmetic; in general, any collection of \gls{fol} formulas) and a \defn{conjecture}.
The axioms and the conjecture are \gls{fol} \defn{formulas} constructed using\todo{Vague?} a common \defn{signature}, that is, a set of non-logical\todo{Is this term ok?} symbols that serve as identifiers of predicates and functions.
...
An \defn{interpretation} of the signature specifies semantics of each of the signature's symbols and, by composition, each formula over the signature.\todo{Point the reader to literature that explains FOL in more detail.}
...
then a finite\todo{?} proof of unsatisfiability exists.
In such case, a proof can be found, for example, by
exhaustively\todo{What if infinitely many can be derived? Then we cannot be exhaustive. We can still be fair, though.}
%systematically\todo{Exhaustively? Fairly?}
...
-- a set of clauses that have been used exhaustively as premises of inference rules to derive new clauses.
%deriving new facts,
...
All new clauses derived by the inferences are added to the unprocessed set.\footnote{If a new clause is subsumed (implied) by another clause derived previously, the new clause may be discarded without loss of completeness. This optimization, known as forward subsumption, is widely used in practice.}\todo{Is it ok to dub subsumption as implication?}\todo{Cite a text that explains forward subsumption.}
...
i.e., it selects either a negative literal or all literals that are maximal with respect to a given \gls{to}\todo{What does this mean? What if there are two incomparable literals in the clause? How does the selection deal with negation?},
...
\Glspl{gnn} admit such name-agnostic\todo{Alternatives: "naming-agnostic", "name-oblivious"} representation naturally \cite{DBLP:conf/cade/JakubuvCOP0U20}.
...
For example, the task of estimating a good age-weight ratio can be implemented as a supervised learning task, provided we have a ground truth\todo{new term} available.
...
\todo[inline]{What is the state of the art in strategy scheduling?}
...
\begin{enumerate}
\item Superposition inference rule, a crucial inference rule of the superposition calculus, is a generalization of term rewriting\todo{potentially obscure}.
The term ordering orients some of the equations and the oriented equations are only used as rewriting rules\todo{Alternative: "rewrite rules"} in one of the two possible directions.
\item In each clause, inferences are restricted to the selected literals --
either one of the negative literals\todo{Which one?}, if there are any in the clause,
or all literals that are maximal with respect to the \gls{to}\todo{What if there are two incomparable literals?}.\todo{Under what conditions exactly does the term ordering affect the literal selection?}
\end{enumerate}
In these two ways, the \gls{to} prunes and guides\todo{Is "guidance" an overstatement?} the proof search.

\Gls{kbo}, the default \gls{to} scheme used in \gls{vampire}, is parameterized by a \defn{\gls{sp}}\todo{It is also parameterized by symbol weights. Should we mention this here?} -- a permutation of the \gls{signature} of the input problem.
...
I present a \defn{\gls{SymbolPrecedenceRecommender}} that sorts the symbols by a linear combination\todo{Stress the challenge of designing the proxy task. "We present a p. recommender which uses ... over syntactic features.} of their syntactic features using learned weights.

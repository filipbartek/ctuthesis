...
for example,
\gls{kbo} and \gls{lpo}.
Both of these \gls{to} schemes are parameterized by a \gls{sp} -- a permutation of the symbols in the signature.\todo{Most of this has been discussed above.}
...
including the arity and the number of occurrences.
Once the system has been trained,
a precedence for an input problem is constructed in two steps:
\begin{enumerate}
...
This value expresses the degree to which the first of the symbols should precede the other.
...
The trained system increased the performance of a baseline Vampire by \pc{6.6},
...
The dataset is of potential interest for research on static algorithm scheduling and \gls{as}.\todo[color=green]{new paragraph}
...
As long as the classes are relatively homogeneous in the performance of the strategies,

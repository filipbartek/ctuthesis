...
In each iteration, one clause is selected from the \defn{unprocessed set} and moved into the \defn{processed set}.
Then, all the admissible inferences within the processed set\todo{Possibly better: \enquote{all the inferences in which processed clauses take part as premises}} are performed,
...
Two main choice points guide \gls{saturation}-based proof search:
\begin{enumerate}
\item Which of the unprocessed clauses should be selected?
\item What inferences should be performed?
\end{enumerate}
My research has dealt with both of these:
See \cref{sec:contrib:ClauseSelection} and \cref{sec:contrib:SymbolPrecedenceRecommenders}, respectively.\todo{Consider leaving out this sentence as it breaks the flow.}

...
\Gls{fol} is a formal language that allows modeling facts and formally reasoning about them.
A formula in \gls{fol} is defined recursively:
...
\alt $\lnot$ <formula>
\alt $\forall$ <variable> <formula>
\alt $\exists$ <variable> <formula>
\alt <atom>

<atom> ::= <predicate> | <predicate> `(' <term-list> `)'

<term> ::= <function> | <function> `(' <term-list> `)' | <variable>

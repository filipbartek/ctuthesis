...
Each of the choice points in a theorem prover is typically governed by a \defn{heuristic} -- a procedure that aims to decide the choice in a way that optimizes the performance of the prover on the input problems of interest.
...
%\todo{Too technical?}
Many such heuristics are parameterized and their configuration can be adjusted to further specialize the prover to a particular type of problems.
...
and, in general, on the configuration of the remaining heuristics.
...
Modern computing technology has opened possibilities for automating expert decisions in various domains by \defn{\gls{ml}}\todo{Explain what \gls{ml} is.} \cite{}.
%and a growing number of researchers believes in the potential of applying \gls{ml} to \gls{atping}.
To apply \gls{ml} in \gls{atping}, however, special considerations need to be taken.
...
such that the prover is likely to solve the problem.
To train such a \gls{ml} system using supervised learning\todo{new term},
...
we need to represent the input problems (sets of \gls{fol} formulas) in a form suitable for \gls{ml}.
For example, we could use a numeric vector consisting of human-designed features, such as symbol counts or term walk counts, or automate the feature extraction using a \gls{rnn} or a \gls{gnn}.

Since the satisfiability of a set of clauses does not depend on the particular naming of the symbols and variables,
we may decide make the representation of the problem oblivious to the names
to make sure we are learning general patterns based on common abstract sub-structures in the problems,
rather than idiosyncratic patterns based naming conventions.
\Glspl{nn} admit such representation naturally.
...
While our final goal is to optimize the performance of the prover in terms of, for example, the percentage of problems solved with a fixed time limit,
for the purpose of training,
we typically need to approximate this measure by a proxy task, for example, regression of age-weight ratio.
\todo{Stress the challenge of ML.}
...
For example, \gls{ml}-based systems have been trained to perform clause selection and axiom selection \cite{DBLP:conf/birthday/BlaauwbroekCGJK24}.
...
Proxy is only intuitively representative.
...
Furthermore, they are potentially useful in other types of parameterized problem solving, as detailed below.
...
\item Literal selection restricts the inferences on any given clause to a subset of literals, and the term ordering affects which literals are selected\todo{How? For example, can only a strictly maximal literal be selected?}.
\end{enumerate}
In these two ways, the \gls{to} prunes and guides\todo{Is "guidance" an overstatement?} the proof search.

\Gls{kbo}, the default term ordering scheme used in \gls{vampire}, is parameterized by a \defn{\gls{sp}} -- a permutation of the \gls{signature} of the input problem.
...
In my initial paper \cite{DBLP:conf/cade/Bartek020} (see \cref{sec:results:simple}), I present a \gls{PrecedenceRecommender} that sorts the symbols by a linear combination\todo{Stress the challenge of designing the proxy task. "We present a p. recommender which uses ... over syntactic features.} of their syntactic features with learned weights.
...
which means ...\todo{Finish the sentence.}
...
which solves the challenge of misaligned signatures present in \gls{tptp}.\todo{This is common with precedence recommender.}

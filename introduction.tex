...
Each of the choice points described previously is typically governed by a \defn{heuristic} -- a procedure that aims to decide the choice in a way that optimizes the performance of the prover on the input problems of interest.
...
%\todo{Too technical?}
Many such heuristics are parameterized and their configuration can be adjusted to further specialize the prover to a particular set or distribution of problems.
...
the optimum value depends on the input problem \cite{DBLP:conf/cade/SchulzM16}

...
For example, the task of estimating a good age-weight ratio is easy to implement as supervised learning task, provided we have some ground truth\todo{new term} available.
...
Predicting an age-weight ratio close to the ground truth is a proxy of solving the problem.
...
For example, \gls{ml}-based systems have been trained to perform clause selection \cite{DBLP:conf/birthday/BlaauwbroekCGJK24}.
Such \gls{ml} system evaluates each inferred clause for the purpose of clause selection.
...
The work presented in this thesis explores several novel approaches, as detailed in the next section.\todo{Describe what is unique about our approach concisely. "A unifying theme for these approaches was", "An interesting commonality of the first two ... is they all train ranking some objects as proxy task."}

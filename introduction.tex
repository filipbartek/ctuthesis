...
\todo[inline]{Consider quoting Diaspora -- knowledge mines.}
...
such as predicate logic and \gls{fol}.\todo{What are the limitations of the encodings?}\todo{Demonstrate on examples.}
Deductive reasoning offers clear-cut solutions\todo{vague} to formalized problems,
and each such solution is justified by a proof.

\Gls{sat} solvers have shown great success and practical applicability in solving problems formulated in \gls{PropositionalLogic} \cite{}.
This challenges the intuition that \acrshort{np}-hardness makes \gls{sat} impractically hard.

The \gls{fol} paradigm offers higher expressive power and more compact formulations,
which makes it suitable for problems in high-level domains such as mathematics and software verification.\todo{Express idea: FOL prover is a general-purpose solver.}\todo{Consider mentioning \gls{hol}.}
However, the expressivity comes at the cost of increased complexity\todo{vague} of reasoning.
The complexity of \acrlong{fol} is epitomized by the undecidability of the logic \cite{}.
...
A \defn{\gls{saturation}-based \gls{tper}} searches for a refutation
(a trivial contradiction represented by the empty clause)
by iteratively deriving new facts,
starting with the input problem expressed in (refutational\todo{Is there a better term?}) \gls{cnf}.
...
All new clauses derived by the generating inferences are added to the unprocessed set.
...
To combat such slowdown, the provers restrict the inferences in various ways.
...
the restriction preserves completeness of the calculus \cite{DBLP:conf/cav/KovacsV13}\todo{Cite the original source.}.
...
that is, functions that were instantiated, by hand or automatically, to optimize the performance of the prover on a class of input problems.
Automated optimization of the heuristics amounts to learning from proof searches.
